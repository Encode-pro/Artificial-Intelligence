...
int
isValid (node * nodeList, const int count, string s1, string s2, string s3)
{
...
bool
permutation (int count, node * nodeList, int n, string s1, string s2,
	     string s3)
{
  if (n == count - 1)
	  ...
	  if (use[i] == 0)
		...
		{		//check validation
		  ...
		  for (int j = 0; j < count; j++)	//print code, which are assigned
		    cout << " " << nodeList[j].letter << " = " << nodeList[j].
		      value;
		  return true;
		}
	    }
...
bool
solvePuzzle (string s1, string s2, string s3)
{
